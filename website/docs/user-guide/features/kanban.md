...
| `kanban.capability_fallback_profiles` | unset (no fallback) | Explicit source→target profile map used only when a ready/review card's assigned profile cannot resolve a forced skill or inference runtime. The dispatcher checks capabilities before claim. Without an authorized, capable target it blocks the card with `kind=capability` and an actionable reason. A successful route changes only the assignee, records one `capability_fallback` provenance event, preserves the task/model/provider overrides, and never chains to a second fallback. |
